...
class ClientA(Client):
    ...
    def task_2(self, client_C_name):
        ...
        grad = np.asarray([self.private_key.decrypt(x) for x in encrypted_dJ_a])
        grad = clip_gradient(grad, self.clip_norm)
        grad = add_gaussian_noise(grad, self.clip_norm, self.epsilon)
        encrypted_masked_dJ_a = [self.public_key.encrypt(x) for x in grad]
        dt.update({"mask": encrypted_masked_dJ_a})
...
class ClientB(Client):
    ...
    def task_2(self,client_C_name):
        try:
            ...
            grad = np.asarray([self.private_key.decrypt(x) for x in encrypted_dJ_b])
            grad = clip_gradient(grad, self.clip_norm)
            grad = add_gaussian_noise(grad, self.clip_norm, self.epsilon)
            encrypted_masked_dJ_b = [self.public_key.encrypt(x) for x in grad]
            dt.update({"mask": encrypted_masked_dJ_b})
        except Exception as e:
        ...
        try:
            ...
            print("B step 2 exception: %s" % e)
        data_to_C = {"encrypted_masked_dJ_b": dt["mask"], "encrypted_loss": encrypted_loss}
        self.send_data(data_to_C, self.other_client[client_C_name])
...
class ClientC(Client):
    ...
    def task_1(self, client_A_name, client_B_name):
        try:
            ...
            print("C step 1 error 1,fail to generate keys: %s" % e)
...
# 计算局部敏感度
def compute_local_sensitivity(grad, clip_norm):
    return clip_norm

# 为每个样本添加高斯噪声
def add_gaussian_noise(grad, clip_norm, epsilon, delta):
    local_sensitivity = compute_local_sensitivity(grad, clip_norm)
    sigma = np.sqrt(2 * np.log(1.25 / delta)) * local_sensitivity / epsilon
